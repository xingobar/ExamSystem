...
@section('content')
    <div class="container">
                ...
                <table class="responsive-table">
                    ...
                    @foreach($positions as $position)
                        <tr>
                            ...
                            <td data-title="location_name">{{$position->location->name}}</td>
                            <td data-title="name">{{$position->name}}</td>
                            <td data-title="edit_location">
                                <a href="#" data-toggle="modal" data-target="#{{$position->id}}"
                                   style="padding-right:20px;">
                                    <i class="fa fa-pencil-square-o" aria-hidden="true"
                                       style="font-size: 2em;color:#bd832d"></i>
                                </a>
                                <a href="/delete_position/{{$position->id}}">
        ...
        @foreach($positions as $position)
            <div id="{{$position->id}}" class="modal fade" role="dialog">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="/update_position/{{$position->id}}}" method="post" class="form-horizontal">
                            {{csrf_field()}}
                                    ...
                                    <div class="col-md-5">
                                        <select name="location_id" class="form-control">
                                            @foreach($locations as $location)
                                                <option value="{{$location->id}}" {{$location->id === $position->location_id ? 'selected': ''}}>{{$location->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
